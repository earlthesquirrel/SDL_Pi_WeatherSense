...
def update_RadMon(CPM):
    ...
    if (updateWebConfig.RADMONUSER != ""):
        myCommand = "radmon.php?function=submit&user=" + updateWebConfig.RADMONUSER + \
            "&password=" + updateWebConfig.RADMONPASSWORD + "&value=" + str(CPM) + "&unit=CPM"
...
def update_GMCMap(CPM, uSVh):
    ...
    if (updateWebConfig.GMCMAPUSERACCOUNTID != ""):
        ...
        myCommand = "log2.asp?AID=" + updateWebConfig.GMCMAPUSERACCOUNTID + "&GID=" + \
            updateWebConfig.GMCMAPGEIGERCOUNTERID + "&CPM=" + str(CPM) + "&uSV=" + str(uSVh)
        print("myCommand=", myCommand)
